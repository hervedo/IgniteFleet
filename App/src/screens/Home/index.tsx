...
export function Home() {
    ...
    async function fetchHistoric() {
        try {
            const resultArrival = historic.filtered("status = 'arrival' SORT(created_at DESC")

            const lastSync = await getLastAsyncTimestamp()

            const formattedHistoric = resultArrival.map((item) => {
                return ({
                    id: item._id.toString(),
                    licensePlate: item.license_place,
    ...
    return (
        <Container >
            ...
            <Content>
                ...
                <FlatList data={vehicleHistoric} keyExtractor={item => item.id} renderItem={({ item }) => (
